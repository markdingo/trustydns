...
var mainTestCases = []mainTestCase{
	{"ecs-set",
		false, 100 * time.Millisecond, []string{"-A", "127.0.0.1:62081", "-v",
			"--ecs-set", "10.0.120.0/24", "http://localhost:63080"}, []string{"Starting"}, ""},

	{"ecs-request-prefix",
		false, 100 * time.Millisecond, []string{"-A", "127.0.0.1:62082", "-v",
	...
	{"URL mangling", // Silently runs with a mangled URL
		false, 100 * time.Millisecond, []string{"-A", "127.0.0.1:62083", "localhost"}, []string{}, ""},

	{"URL syntax", // Silently runs with a dodgy (but legal) URL
		false, 100 * time.Millisecond, []string{"-A", "127.0.0.1:62084", "///localhost"}, []string{}, ""},

	{"Good URL with scheme",
		false, 100 * time.Millisecond, []string{"-A", "127.0.0.1:62085", "-v", "http://localhost"},
		[]string{"Starting", "Exiting"}, ""},

	{"Good URL No Scheme",
		false, 100 * time.Millisecond, []string{"-g", "-v", "-A", "127.0.0.1:62086", "localhost"},
		[]string{"Starting", "Exiting"}, ""},

	{"Good local resolver config",
		false, 100 * time.Millisecond, []string{"-v", "-A", "127.0.0.1:62087",
			"-c", "testdata/resolv.conf", "http://localhost"},
		[]string{"Starting", "Exiting"}, ""},

	{"log-all",
		false, 100 * time.Millisecond,
		[]string{"-v", "--log-all", "-A", "127.0.0.1:62088",
			"-c", "testdata/resolv.conf", "http://localhost"},
		[]string{"Starting", "Exiting"}, ""},

	{"Status report",
		false, 2 * time.Second, []string{"-v", "-i", "1s", "-A", "127.0.0.1:62089", "http://localhost"},
		[]string{"Status Server:"}, ""},

	{"CPU Profile",
		false, 100 * time.Millisecond, []string{"-A", "127.0.0.1:62090", "--cpu-profile", "testdata/cpu",
			"http://localhost"}, []string{}, ""},
	{"Mem Profile",
		false, 100 * time.Millisecond, []string{"-A", "127.0.0.1:62091", "--mem-profile", "testdata/mem",
			"http://localhost"}, []string{}, ""},
